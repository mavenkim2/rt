...
float3 SampleBSDF(GPUMaterial material, float3 sample3, float3 reflectance, inout float3 throughput, float3 wo, inout float pdf)
{
    switch (material.type)
    {
        case GPUMaterialType::Diffuse:
        {
            return SampleDiffuse(reflectance.xyz, wo, sample3.xy, throughput);
        }
        case GPUMaterialType::Dielectric:
        {
            return SampleDielectric(wo, material.ior, sample3.xy, throughput);
        }
        case GPUMaterialType::Disney:
        {
            return SampleDisney(material, sample3, reflectance.xyz, throughput, wo, pdf);
        }
    }
    return 0;
}
