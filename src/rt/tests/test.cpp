...
void EdgeStitchTest()
{
    ...
    int maxEdgeRates[] = {Max(edgeRates[0], edgeRates[2]), Max(edgeRates[1], edgeRates[3])};

    TessellatedVertices tessellatedVertices;
    tessellatedVertices.vertices.push_back(Vec3f(0, 0, 0));
    tessellatedVertices.vertices.push_back(Vec3f(1, 0, 0));
    tessellatedVertices.vertices.push_back(Vec3f(1, 1, 0));
    tessellatedVertices.vertices.push_back(Vec3f(0, 1, 0));

    for (int i = 0; i < 4; i++)
    {
        edges[i].indexStart = (int)tessellatedVertices.vertices.size();
        edges[i].edgeFactor = edgeRates[i];
        edges[i].id0        = i;
        edges[i].id1        = (i + 1) & 3;

        Vec3f start  = tessellatedVertices.vertices[i];
        f32 stepSize = 1.f / edgeRates[i];
        for (int j = 1; j < edgeRates[i]; j++)
        {
            Vec3f v = start + (stepSize * j) *
                                  Vec3f(f32(uvDiffTable[i][0]), f32(uvDiffTable[i][1]), 0.f);
            tessellatedVertices.vertices.push_back(v);
        }
    }

    if (maxEdgeRates[0] == 1 && maxEdgeRates[1] == 1)
    {
        // output two triangles (attached to the corners)
    }
    else
    {
        // Add a point to the center
        if (maxEdgeRates[0] == 1 || maxEdgeRates[1] == 1)
            tessellatedVertices.StartNewGrid(2, 2);
        else tessellatedVertices.StartNewGrid(maxEdgeRates[0], maxEdgeRates[1]);

        for (u32 edgeIndex = 0; edgeIndex < 4; edgeIndex++)
        {
            int edgeRate         = edgeRates[edgeIndex];
            EdgeInfo currentEdge = edges[edgeIndex];
            ...
            int edgeStep   = 0;
            Vec2i gridStep = Vec2i(Max(maxEdgeRates[0] - 2, 0), Max(maxEdgeRates[1] - 2, 0));
